...
test.describe("Contacts", () => {
    ...
    test("create a new contact via form", async ({ page }) => {
        ...
        await page
            .getByTestId("contact-description")
        ...
        await expect(page.getByTestId("contact-full-name")).toHaveText(
            `${firstName} ${lastName}`,
        );
    });
    ...
    test("edit an existing contact", async ({ page }) => {
        ...
        await page.waitForURL(/\/contacts\/\d+$/);
        await expect(page.getByTestId("contact-full-name")).toContainText(
            updatedFirstName,
        );
    });
