...
test.describe("Invitations", () => {
    test("complete journey: send and accept invitation", async ({
        browser,
    }) => {
        ...
        await expect(
            page.getByText(/Invitation sent successfully/i),
        ).toBeVisible();
        ...
        const acceptBtn = page
            .locator('button[data-testid^="invitation-accept-"]')
            .first();
        await acceptBtn.click();

        await expect(
            page.getByText(/Invitation accepted successfully/i),
        ).toBeVisible();
        ...
        await page.waitForURL(`/${ORG_SLUG}/dashboard`);
        await expect(
            page.getByRole("heading", { name: "Nexus Corp" }),
        ).toBeVisible();
    ...
    test("journey: send and decline invitation", async ({ browser }) => {
        ...
        const declineBtn = page
            .locator('button[data-testid^="invitation-decline-"]')
            .first();
        await declineBtn.click();

        await expect(
            page.getByText(/Invitation declined successfully/i),
        ).toBeVisible();
    ...
    test.describe.serial("Validation", () => {
        test("user does not exist", async ({ page }) => {
            ...
            await page.goto(`/${ORG_SLUG}/members/create`);
            await page
                .getByTestId("invitation-member-info")
                .fill("truly-non-existent-user-12345@example.com");
            await page.getByTestId("invitation-submit").click();
