...
test.describe('Form Validations', () => {
  ...
  test('company creation shows validation errors', async ({ page }) => {
    ...
    await expect(page.locator('text=/name.*required/i').first()).toBeVisible();
    await expect(page.locator('text=/industry.*required/i').first()).toBeVisible();
  });
  ...
  test('contact creation shows validation errors', async ({ page }) => {
    ...
    const errors = page.getByTestId('input-error');
    await expect(errors.first()).toBeVisible({ timeout: 30000 });
    await expect(page.locator('text=/first name.*required/i').first()).toBeVisible();
    await expect(page.locator('text=/last name.*required/i').first()).toBeVisible();
